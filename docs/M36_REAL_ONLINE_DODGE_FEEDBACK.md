...
- Each bounded attempt starts Chrome's genuine LMB attack first, then 35 ms into the 135 ms authoritative windup sends Firefox the genuine W3C `KeyS` + Space dodge sequence with no artificial pre-delay. M24 already owns reaction-timing proof; M36 instead keeps enough scheduling margin for headless Firefox to sample and transmit the one-shot Dodge before strike-active. The critical iframe/strike window contains no evidence polling. A timing miss may retry only while both fighters remain at 100 HP / 100 guard and no parry feedback appears; any resolved damage or parry fails closed. Before a clean retry, ordinary `KeyW` movement approximately unwinds the preceding perpendicular Dodge displacement so the next exchange starts inside the same authoritative threat geometry. Fresh attack-commit and opposite-client dodge feedback are scoped to the successful attempt, so stale evidence cannot satisfy a retry.
...
CI #182 and its exact-head rerun both reproduced a different scheduling failure: Firefox recorded genuine `KeyS` + Space down/up provenance, but the one-shot Dodge request was sampled too late and Chrome's real strike resolved as 34 HP damage. Because M24 already proves dodge reaction timing, M36 now removes the cross-driver race: Chrome's genuine attack begins first and Firefox receives the same genuine Dodge request 35 ms into windup. No combat constants, iframe duration, damage rules, feedback rules, or fail-closed vitals checks are changed.
